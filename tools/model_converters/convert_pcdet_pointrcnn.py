...
def convert_rpn_head(model_key, model_weight, state_dict, converted_names):
    ...
    if task_layer == 'cls_layers':
        if block_id in conv_list_set:
            ...
            task_layers = 'conv_cls'
            new_key = f'{head}.conv_pred.{task_layers}.{module}'
    else:
        if block_id in conv_list_set:
            task_layers = 'reg_convs'
            block_name = f'layer{int(block_id / 3)}'
            module = 'conv.weight'
            new_key = f'{head}.conv_pred.{task_layer}.{block_name}.{module}'
        elif block_id < 6:
            ...
            task_layers = 'conv_reg'
            new_key = f'{head}.conv_pred.{task_layers}.{module}'
    state_dict[new_key] = model_weight
    converted_names.add(model_key)

def convert_roi_head(model_key, model_weight, state_dict, converted_names):
    new_key = model_key.replace('roi_head', 'roi_head.bbox_head')
    ...
    if split_keys[1] == 'SA_modules':
        ...
        block_id = int(int(block) / 2)
        module = 'conv.' + module
        new_key = f'{roi_head}.{merge_down_layer}.{block_id}.{module}'
    else:
        head, task_layer, block, module = split_keys
        block_id = int(block)
...
def main():
    checkpoint = torch.load('./work_dirs/pointrcnn_2x8_kitti-3d-car/pointrcnn_7870.pth')
    pointrcnn = torch.load('./work_dirs/pointrcnn_2x8_kitti-3d-car/test/latest.pth')
    ...
    torch.save(save_checkpoint, './work_dirs/pointrcnn_2x8_kitti-3d-car/convert_pointrcnn_7870.pth')
    '''
    print('-------------------------PointRCNN-------------------------')
